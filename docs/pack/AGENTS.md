...
Read [00_START_HERE](00_START_HERE.md), [decisions](02_DECISIONS.md), [repository map](03_REPOSITORY_MAP.md), and your assigned [goal](delivery/GOAL_INDEX.md). Use the source register for exact upstream files. This is a documentation package for a proposed new repository, not an assertion that its source modules already exist.
...
Return changed files and actual commit/tree, commands run and results, user-visible behavior exercised, remaining limitations, unresolved effects, source IDs consulted and next action. Use [the session handoff](templates/SESSION_HANDOFF.md). A pending permission, uncertain deployment or failed check stays pending/uncertain/failed.
