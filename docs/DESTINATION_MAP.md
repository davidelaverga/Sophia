...
| `packages/contracts/` | built | S1-02, S1-04, S1-04A, S1-05A | OpenAPI contract = the pack plus reviewed JSON Patch amendments (A01 room, A02 room access, A03 lobby decline and block, A04 runtime service, A05 discussion and native tasks, A06 room exchange and media bridge routes, A07 durable room removal), generated types, generated response validators (`./validate`), the dsh bundle's runtime wire module, Ajv component schemas, SSE frame parser |
...
| `apps/worker/` | partial | S1-05A, S1-06, S1-08, S1-11, S1-13 | Built (S1-05A): runtime dispatch on the sophia_worker login (claim, recheck, queue or deny, reconcile uncertain rows), and room removal reconciliation (a declined or blocked guest is taken out of the call on the LiveKit server's evidence, retried with backoff, watched while an earlier token lives). Later goals add the other job consumers and the scheduler |
...
| `db/migrations/` | built | S1-02, S1-04, S1-04A, S1-05A | Pack 0001–0004 verbatim plus 0005–0016 (event notify, idempotent project creation, dispatch fencing, idempotent dispatch results, project room and input floor, room access: invitations, lobby, sessions; lobby decline for now and block for good; runtime service, discussion and draft_brief; the room exchange, bridge presence, guest quiescence and holder departure; durable room removals; a guest on their way in counts as present; results under current authority, a locked command poll, eligible decisions, a padded removal watch); no automatic old-DB migration |
