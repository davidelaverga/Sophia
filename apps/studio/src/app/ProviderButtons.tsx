// "Continue with Google / GitHub" above the email form. Each provider leaves for its own consent page and
    ...
    <>
      <div className="providers">
        {oauthProviders.map((p) => (
          <button key={p} type="button" className="provider" disabled={leaving !== null} onClick={() => void go(p)}>
            <ProviderMark provider={p} />
            {leaving === p ? `Opening ${PROVIDER_NAME[p]}…` : `Continue with ${PROVIDER_NAME[p]}`}
          </button>
        ))}
      </div>
      {error && (
