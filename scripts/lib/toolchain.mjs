...
export function pinnedToolchain() {
  ...
  if (pkg.engines?.node !== nodeVersion)
    throw new Error(`engines.node ${pkg.engines?.node} differs from .node-version ${nodeVersion}`)
  if (pkg.engines?.pnpm !== pnpm)
    throw new Error(`engines.pnpm ${pkg.engines?.pnpm} differs from packageManager ${pnpm}`)
...
export function activeToolchain() {
  ...
  return {
    ...
    pnpm: pnpm.status === 0 ? pnpm.stdout.trim() : `unavailable (${pnpm.stderr.trim()})`,
  }
}
...
export function assertToolchain() {
  ...
  const problems = []
  if (active.node !== pinned.node)
    problems.push(`node ${active.node} is active; this repository requires ${pinned.node} (.node-version)`)
  if (active.pnpm !== pinned.pnpm)
    problems.push(`pnpm ${active.pnpm} is active; this repository requires ${pinned.pnpm} (packageManager)`)
