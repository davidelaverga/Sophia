...
const placeholders = { [layout.root]: '<INSTALL>', [RUNTIME_DIR]: '<RUNTIME>', [REPO_ROOT]: '<REPO>', [dirname(process.execPath)]: '<NODE_BIN>' }
...
save('install.log', `$ dsh plugin --profile ${unit.dsh.profile} install --frozen-lockfile --offline\n${install.stdout}${install.stderr}`)
...
console.log(`composition ${gate.ok ? 'verified' : 'REJECTED'}; healthy=${gate.health.healthy} (${gate.health.reasons.join('; ')})`)
save('dump-config.yml', gate.dump.stdout)
save('dump-config.stderr.txt', gate.dump.stderr)
if (evidence) writeJson(join(evidence, 'gate.json'), JSON.parse(norm(JSON.stringify({ ok: gate.ok, health: gate.health, checks: gate.checks, dump_status: gate.dump.status }))))
...
if (values.boot) {
  ...
  console.log(`boot: ${stillRunning ? `running at the ${seconds}s deadline; SIGTERM disposed it (exit ${boot.status ?? boot.signal})` : `exited on its own before the deadline (${boot.status ?? boot.signal})`}`)
  ...
  for (const log of boot.diagnostics) {
    ...
    if (evidence) { mkdirSync(join(evidence, 'startup-diagnostics'), { recursive: true }); copyFileSync(log, join(evidence, 'startup-diagnostics', log.split('/').at(-1))) }
  }
  save('boot.stderr.txt', boot.stderr)
  save('boot.stdout.txt', boot.stdout)
  if (evidence) writeJson(join(evidence, 'boot.json'), { seconds, exit_status_after_sigterm: boot.status, signal: boot.signal, still_running_at_deadline: stillRunning, bridge_line: boot.bridgeLine, startup_diagnostics: boot.diagnostics.map((p) => norm(p)) })
}
